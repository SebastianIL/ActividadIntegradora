...
plt.figure()
plt.plot(range(1, 17), resultados)
plt.title('Pasos necesarios para que todo los coches lleguen a su destino')
plt.suptitle('(1000 repeticiones)')
plt.xlabel('Número de coches')
plt.ylabel('Número promedio de pasos')
plt.show()

plt.figure()
plt.plot(range(1, 17), longitudes, label="Pasos de Dijkstra", color='red')
plt.plot(range(1, 17), pasos_reales, label="Pasos Realizados", color='blue')
